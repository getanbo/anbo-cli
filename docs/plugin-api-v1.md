...
Target requests include API version `1`, action, stable logical/runtime project IDs, namespaced config, parsed arguments and flags, plus `passthrough` for tokens after `--`. The `args` array also retains the `--` marker for compatibility.
...
- `process`: shell-free child processes with capture, timeout, and cancellation.
